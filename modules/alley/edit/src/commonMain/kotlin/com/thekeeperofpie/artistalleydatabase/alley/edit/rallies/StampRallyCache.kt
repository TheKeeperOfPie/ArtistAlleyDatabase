...
@Inject
class StampRallyCache(
    applicationScope: ApplicationScope,
    private val database: AlleyEditDatabase,
) {
    private val refreshFlow = RefreshFlow()
    val stampRalliesAnimeExpo2026 = refreshFlow.updates
        .mapLatest { database.loadStampRallies(DataYear.ANIME_EXPO_2026).reversed() }
    ...
    fun stampRallies(dataYear: DataYear): Flow<List<StampRallySummary>> = when (dataYear) {
        ...
        DataYear.ANIME_NYC_2025,
            -> flowFromSuspend { database.loadStampRallies(dataYear).reversed() }
    }
