...
fun ScrollableColumn(content: @Composable ColumnScope.() -> Unit) {
    ...
    ScrollArea(state = scrollAreaState) {
        Column(
            ...
            Column(modifier = Modifier.widthIn(max = 960.dp).verticalScroll(scrollState)) {
                content()
                Spacer(Modifier.height(80.dp))
            }
        }
...
@Composable
fun LazyColumnSideBySide(
    showSecondary: () -> Boolean,
    primary: LazyListScope.() -> Unit,
    secondary: LazyListScope.() -> Unit,
    secondaryExpanded: @Composable () -> Unit,
    modifier: Modifier = Modifier,
    listStateOne: LazyListState = rememberLazyListState(),
) {
    val scrollAreaStateOne = rememberScrollAreaState(listStateOne)
    ...
    if (!canShowExpanded) {
        ScrollArea(state = scrollAreaStateOne, modifier = modifier) {
            Column(
                ...
                LazyColumn(state = listStateOne, modifier = Modifier.widthIn(max = 960.dp)) {
                    if (showSecondary()) {
                        secondary()
                    }
                    primary()
                    item("bottomSpacer") { Spacer(Modifier.height(80.dp)) }
                }
            }
