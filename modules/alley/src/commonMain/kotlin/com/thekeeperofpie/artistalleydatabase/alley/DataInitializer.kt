...
class DataInitializer(
    ...
    private suspend fun parseArtists() {
        ...
        openFile("files/$ARTISTS_CSV_NAME").use {
            var counter = 1
            CsvReader.read(it)
                .map {
    ...
    private suspend fun parseTags() {
        ...
        openFile("files/$SERIES_CSV_NAME").use {
            CsvReader.read(it)
                .map {
                    // Series, Notes
                    val name = it["Series"]!!
        ...
        openFile("files/$MERCH_CSV_NAME").use {
            CsvReader.read(it)
                .map {
                    // Merch, Notes
                    val name = it["Merch"]!!
    ...
    private suspend fun parseStampRallies() {
        ...
        openFile("files/$STAMP_RALLIES_CSV_NAME").use {
            var counter = 1
            CsvReader.read(it)
                .mapNotNull {
    ...
    private suspend fun openFile(path: String) = Buffer().apply { write(Res.readBytes(path)) }
