...
suspend fun <ActivityEntry> applyActivityFiltering(
    ...
    if (!filteringData.showAdult && mediaFilterable?.isAdult == true) return null
    var copiedEntry = entry
    if (mediaFilterable != null) {
        val mediaId = mediaFilterable.mediaId
        ...
        if (mediaListStatuses.containsKey(mediaId)) {
            ...
            status = mediaFilterable.mediaListStatus
            progress = mediaFilterable.progress
            progressVolumes = mediaFilterable.progressVolumes
        }
