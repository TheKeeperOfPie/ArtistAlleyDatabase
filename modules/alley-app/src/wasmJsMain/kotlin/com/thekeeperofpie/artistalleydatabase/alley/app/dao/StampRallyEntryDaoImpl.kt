...
import com.thekeeperofpie.artistalleydatabase.alley.app.StampRallyEntryQueries
...
import com.thekeeperofpie.artistalleydatabase.alley.rallies.StampRallyWithArtistsEntry
...
class StampRallyEntryDaoImpl(
    ...
    override suspend fun getEntryWithArtists(id: String) =
    ...
    override suspend fun insertEntries(vararg entries: StampRallyEntry) =
        insertEntries(entries.toList())

    override suspend fun insertEntries(entries: List<StampRallyEntry>) =
        dao().transaction {
            entries.forEach {
    ...
    override suspend fun clearEntries() = dao().clear()

    override suspend fun clearConnections() = dao().clearArtistConnections()

    override suspend fun insertConnections(entries: List<StampRallyArtistConnection>) =
        dao().transaction {
            entries.forEach {
    ...
    override suspend fun retainIds(ids: List<String>) =
    ...
    override suspend fun insertUpdatedEntries(entries: Collection<Pair<StampRallyEntry, List<StampRallyArtistConnection>>>) {
        dao().transaction {
