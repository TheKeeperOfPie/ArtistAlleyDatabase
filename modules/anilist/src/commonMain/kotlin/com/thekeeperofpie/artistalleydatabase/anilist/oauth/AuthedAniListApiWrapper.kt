...
class AuthedAniListApiWrapper(
    ...
    override suspend fun mediaDetails(id: String, skipCache: Boolean) =
        super.mediaDetails(id, skipCache).let {
    ...
    override suspend fun mediaDetails2(id: String, skipCache: Boolean) =
        super.mediaDetails2(id, skipCache).let {
            val media = it.result?.media
            if (media != null && media.isAdult != false) throw IOException("Cannot load this media")
            it.transformResult {
                it.copy(
    ...
    override suspend fun characterDetails(id: String, skipCache: Boolean) =
        super.characterDetails(id, skipCache).let {
            it.transformResult {
                it.copy(
    ...
    ) = super.anime2AnimeConnectionDetails(mediaId, characterIds, voiceActorIds, staffIds).also {
